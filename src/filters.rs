    ...
    impl Filter {
        ...
        fn get_user_value(
            ...
            match sub_type {
                ...
                constants::SUB_TYPE_PLATFORM => Some(serde_json::Value::String(user.platform_data.platform.clone())),
                constants::SUB_TYPE_PLATFORM_VERSION => Some(serde_json::Value::String(user.platform_data.platform_version.clone())),
                constants::SUB_TYPE_APP_VERSION => Some(serde_json::Value::String(user.app_version.clone())),
                constants::SUB_TYPE_DEVICE_MODEL => Some(serde_json::Value::String(user.device_model.clone())),
                constants::SUB_TYPE_CUSTOM_DATA => {
                    ...
                    if !self.values.is_empty() {
                        if let Some(key) = self.values[0].as_str() {
                            user.custom_data.get(key).cloned()
            ...
            for filter_value in &self.values {
                let matches = match comparator {
                    ...
                    constants::COMPARATOR_CONTAIN => {
                        if let (serde_json::Value::String(user_str), serde_json::Value::String(filter_str)) = (user_value, filter_value) {
                    ...
                    constants::COMPARATOR_GREATER => {
                        if let (serde_json::Value::Number(user_num), serde_json::Value::Number(filter_num)) = (user_value, filter_value) {
        ...
        fn evaluate_audience_match(
            ...
            for audience_id in &self._audiences {
                if let Some(audience) = audiences.get(audience_id) {
                    if audience.filters.evaluate(audiences, user, client_custom_data) {
                        return true;
                    }
                }
            }
